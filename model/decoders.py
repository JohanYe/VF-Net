...
class FoldNet_Decoder_stochman(nn.Module):
    def __init__(self, args, num_points):
        ...
        self.folding1 = nn.Sequential(
            nn.Linear(args.feat_dims + 2 + (self.shape != 'plane'), args.feat_dims),
            nn.ReLU(),
            nn.Linear(args.feat_dims, args.feat_dims),
            nn.ReLU(),
            nn.Linear(args.feat_dims, args.feat_dims),
            nn.ReLU(),
            nn.Linear(args.feat_dims, 3),
    ...
    def forward(self, latent_codes, feat1, sample_grid=False, edge_only=False):
        ...
        if self.point_encoding:
            ...
            return self.decode(latent_codes, self.grid)  # (batch_size, num_points ,3)
        else:
            m = self.m if not edge_only else int(self.meshgrid[0][-1]) * 2 + int(self.meshgrid[1][-1]) * 2
            ...
            output = self.decode(latent_codes, self.grid)  # (batch_size, num_points ,3)
