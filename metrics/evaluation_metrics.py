...
def emd_cd(sample_pcs, ref_pcs, batch_size, accelerated_cd=True, reduced=True):
    ...
    cd_lst = list()
    emd_lst = list()
    for b_start in tqdm(range(0, n_sample, batch_size)):
...
# ALL
def compute_all_metrics(sample_pcs, ref_pcs, batch_size=32, accelerated_cd=True):
    with torch.no_grad():
        results = {}
        # MMD, COV
        ...
        print(batch_size)
        print(M_rr_cd.size(), M_rs_cd.size(), M_ss_cd.size())
        one_nn_cd_res = knn(M_rr_cd, M_rs_cd, M_ss_cd, 1, sqrt=False)
        results.update({"1-NN-CD-%s" % k: v for k, v in one_nn_cd_res.items() if 'acc' in k})
        print(M_rr_emd.size(), M_rs_emd.size(), M_ss_emd.size())
